...
def server_non_host_docker(repo, ssh, name="", volume="", volume_target="/root", share_path=None, share_path_target="/root"):
    if volume is not None:
        if volume_target is not None:
        ...
        ssh.exec_command("docker run -it --network host --name {0} {1} /bin/bash"
                  .format(name, repo))
